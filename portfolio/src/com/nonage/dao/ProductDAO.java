...
public class ProductDAO {
	...
	public ArrayList<ProductVO> listNewProduct() {
		...
		try {
			...
			while (rs.next()) {
				...
				product.setPrice2(rs.getInt("price2")); // 판매가
	...
	public int totalRecord(String product_name) {
		int total_pages=0;
		String sql = "select count(*) from product where name like '%'||?||'%'"; //총 개수 가져오기
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	static int counts = 5; //한페이지에 나타낼 상품의 개수
