...
function go_save()
{
	...
	if (theForm.kind.value == '') {
		...
		// 디비에 추가하기 위한 페이지인 product_Write.jsp로 이동하되
...
function go_mod_save(tpage, pseq) {
	...
	if (theForm.kind.value == '') {
		...
		theForm.content.focus();
			...
			if (confirm('수정하시겠습니까?')) {
				// [1] 상품을 삭제하지 않는 대신 사용하지 않음을 products
				//테이블의 useyn 컬럼에 1을 채워 넣기 위해서
...
function order_search(){
	var theForm=document.frm;
	theForm.action = "NonageServlet?command=admin_order_list";
	theForm.submit();
}
